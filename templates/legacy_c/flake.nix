{
  ...
  in {
    devShells = forEachSupportedSystem ({pkgs}:
      ...
      "useMold and useGold cannot be enabled at the same time."; let
        ...
        mkShell =
          ...
          then
            (pkgs.mkShell.override {
              stdenv =
                if useMold
                then pkgs.stdenvAdapters.useMoldLinker pkgs.llvmPackages_16.stdenv
                else if useGold
                then pkgs.stdenvAdapters.useGoldLinker pkgs.llvmPackages_16.stdenv
            ...
            (pkgs.mkShell.override {
              stdenv =
                if useMold
                then pkgs.stdenvAdapters.useMoldLinker pkgs.gcc13Stdenv
                else if useGold
                then pkgs.stdenvAdapters.useGoldLinker pkgs.gcc13Stdenv
