...
sub notify { 
	my ($title, $msg) = @_;
	system "notify-send", "--app-name=irssi", "--urgency=normal", "--expire-time=0", $title, $msg;
}
...
sub priv_msg {
	...
	notify("Private message from $nick", $msg_stripped);
}
...
sub hilight {
	...
	if ($dest->{level} & MSGLEVEL_HILIGHT && $dest->{hilight_priority} != $ihl) {
		...
		notify("Mention in $dest->{target}", $stripped);
	}
}
